[1.py](https://github.com/user-attachments/files/27246432/1.py)
"""
=============================================================================
...
with col_data:
    ...
    if n_projects == 0:
        ...
        # Phát hiện thay đổi → đánh dấu stale
        ...
        with btn_del:
            if st.button("Xóa đã chọn", use_container_width=True):
                ...
                if n_del == 0:
                    ...
                    st.session_state["df_projects"] = edited_df[~mask].reset_index(drop=True)
                    st.session_state["result_stale"] = True
                    st.rerun()
    ...
    if n_projects > 0 and "run_btn" in dir() and run_btn:
        df_run = st.session_state["df_projects"]
